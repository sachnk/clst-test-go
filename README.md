...
```sh
go get -u 'github.com/sachnk/clst-test-go@v0.1.0-alpha.1'
```
...
func main() {
	client := clsttest.NewClient(
		option.WithBearerToken("My Bearer Token"), // defaults to os.LookupEnv("CLST_TEST_BEARER_TOKEN")
	)
...
```go
params := FooParams{
	Name: clsttest.F("hello"),

	// Explicitly send `"description": null`
	Description: clsttest.Null[string](),

	Point: clsttest.F(clsttest.Point{
		X: clsttest.Int(0),
		Y: clsttest.Int(1),

		// In cases where the API specifies a given type,
		// but you want to send something else, use `Raw`:
		Z: clsttest.Raw[int64](0.01), // sends a float
	}),
}
```
...
client.Instruments.Get(context.TODO(), ...,
	// Override the header
...
`*clsttest.Error`. This contains the `StatusCode`, `*http.Request`, and
...
if err != nil {
	...
	panic(err.Error()) // GET "/instruments/{symbol}": 400 Bad Request { ... }
}
```
...
We also provide a helper `clsttest.FileParam(reader io.Reader, filename string, contentType string)`
...
```go
params := FooNewParams{
    ID:   clsttest.F("id_xxxx"),
    Data: clsttest.F(FooNewParamsData{
        FirstName: clsttest.F("John"),
    }),
}
